export * from './regex';

export const ACCESS_TOKEN_NAME =
  'pVCvLw6fxU46dN4.OEM/qujkz4Th4H9a3uLa/agOvIQivXeHUEaWm';
export const REFESH_TOKEN_NAME =
  'OtgM39Tm0ci4yljIBncNROvKIEJK1caM/1jOsOPR1jlfQRJkvJlyW';

export const CASE_SORT = ['TIME_DESC', 'TIME_ASC', 'NAME_ASC', 'NAME_DESC'];

export const MAIL_QUEUE = 'MAIL_QUEUE';
export const MAIL_QUEUE_SUBSCRIBE_CREATE_CONTENT =
  'MAIL_QUEUE_SUBSCRIBE_CREATE_CONTENT';
